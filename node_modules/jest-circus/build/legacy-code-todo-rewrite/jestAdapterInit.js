...
const initialize = async ({
  ...
  const snapshotState = new _State.SnapshotState('', {
    expand: globalConfig.expand,
  ...
  switch (event.name) {
    ...
    case 'test_done': {
      event.test.numPassingAsserts =
        _expect.default.getState().numPassingAsserts;
...
const _addExpectedAssertionErrors = test => {
  const failures = _expect.default.extractExpectedAssertionsErrors();
...
const _addSuppressedErrors = test => {
  const {suppressedErrors} = _expect.default.getState();
  _expect.default.setState({
    suppressedErrors: []
  });
